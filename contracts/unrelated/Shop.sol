...
contract Shop {
    ...
    function SetUser(string memory name, string memory info) public {
        for (uint i = 0; i < o_lenght; i++) {
            require(
                keccak256(abi.encodePacked(owner[i].name)) !=
    ...
    function UserVerification(
        ...
        for (uint i = 0; i < o_lenght; i++) {
            if (owner[i].owner == user) {
                if (
                    keccak256(abi.encodePacked(owner[i].info)) ==
                    keccak256(abi.encodePacked(info))
                ) {
                    return true;
                } else return false;
            }
    ...
    function CountProduct(string memory name) public view returns (uint) {
        ...
        for (uint i = 0; i < p_lenght; i++) {
            if (
                keccak256(abi.encodePacked(product[i].name)) ==
                keccak256(abi.encodePacked(name))
            ) {
    ...
    function CreateProduct(
        ...
        require(
            UserVerification(msg.sender, "Provider") == true,
            "You not Provider"
        );
    ...
    function UpStatusSupplyProduct(uint id_pd) public {
        ...
        require(
            CountProduct(productdel[id_pd].name) > productdel[id_pd].count,
            "Not enough product"
        );
        for (uint i = 0; i < p_lenght; i++) {
            if (
                keccak256(abi.encodePacked(product[i].name)) ==
                keccak256(abi.encodePacked(productdel[id_pd].name))
            ) {
                product[i].mincount =
                    product[i].mincount -
                    productdel[id_pd].count;
            }
        }
    ...
    function CreateSupply(
        ...
        productdel[pd_lenght] = ProductDelivery(
            msg.sender,
            nameprov,
            name,
            price,
    ...
    function UpStatusSaleProduct(uint id_s) public {
        require(
            UserVerification(msg.sender, "Saleman") == true,
            "You not Saleman"
        );
