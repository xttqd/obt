...
contract Mail {
    ...
    function SendMailPlace() private {
        mailplaceA[0] = MailPlace(
            "SenderPostOffice",
            "101000",
            WayPackage.SenderPostOffice
        );
        mailplaceA[1] = MailPlace(
            "MainPostOffice",
            "102000",
            WayPackage.MainPostOffice
        );
        mailplaceA[2] = MailPlace(
            "SortingCenter",
            "103000",
            WayPackage.SortingCenter
        );
        mailplaceA[3] = MailPlace(
            "RecipientPostOffice",
            "104000",
            WayPackage.RecipientPostOffice
        );
    ...
    function TrackNumberGeneration(
        ...
        for (uint q = 0; q < mp_linght; q++) {
            if (
                keccak256(abi.encodePacked((mailplaceA[q].mailplace))) ==
                keccak256(abi.encodePacked((mailplace)))
            ) {
                tracknumbergenerationS[0].indx = mailplaceA[q].inx;
            }
        }
        str = A(
            tracknumbergenerationS[0].a,
            tracknumbergenerationS[0].b,
            tracknumbergenerationS[0].c,
    ...
    function A(
        ...
        string memory _tmpValue = new string(
            bytesa.length + bytesb.length + bytesc.length + bytesindx.length
        );
    ...
    function B(
        ...
        string memory _tmpValue = new string(
            bytesmailplace.length + bytesaddr.length
        );
        bytes memory _newValue = bytes(_tmpValue);
    ...
    function UserAddress(string memory name) private view returns (address o) {
        for (uint i = 0; i < u_linght; i++) {
            if (
                keccak256(abi.encodePacked((user[i].name))) ==
    ...
    constructor() public {
        ...
        owheradmin[0] = OwnerAdmin(
            MainAdmin,
            "Main admin",
            "st. Bold house 5",
            0,
            AdminRole.MainAdminSistem
        );
    ...
    function SetAdmin(
        ...
        balance = 0;
        owheradmin[oa_linght] = OwnerAdmin(
            ownera,
            name,
            homeaddress,
            balance,
            AdminRole.AdminSistem
        );
        oa_linght = oa_linght + 1;
    }
    //
  ...
  //Функция изменения своих данных 
    ...
    function StopMailMailAdmin(string memory tracknumber) public {
        for (uint i = 0; i <= p_linght; i++) {
            if (
                keccak256(abi.encodePacked((package[i].tracknumber))) ==
    ...
    function SetEmployee(
        ...
        string memory name,
        string memory homeaddress,
        string memory idbranch,
        uint balance
    ) public {
        for (uint i = 0; i < oa_linght; i++) {
            if (owheradmin[i].ownera == msg.sender) {
                require(
                    owheradmin[i].role == AdminRole.AdminSistem,
                    "You not main admin!"
                );
            }
        }
        employee[e_linght] = Employee(
            ownera,
            name,
            homeaddress,
            balance,
  ...
  //*/
    ...
    function SendMailA(
        ...
        string memory addresssender
    ) public {
        for (uint i = 0; i < e_linght; i++) {
            if (employee[i].ownere == msg.sender) {
                require(
                    employee[i].role == EmployeeRole.Work,
                    "You not main admin!"
                );
            }
        }
        ...
        package[p_linght] = Package(
            TrackNumberGeneration(p_linght, mailplace),
            UserAddress(sender),
            UserAddress(recipient),
            typepackage,
            AddressPlus(mailplace, addressrecipient),
            AddressPlus(mailplace, addresssender),
            false,
            WayPackage.SenderPostOffice
        );
    }
    ...
    function SendWayPackage(string memory tracknumber) public {
        ...
        for (i = 0; i <= mp_linght; i++) {
            if (
                keccak256(abi.encodePacked((mailplaceA[i].inx))) ==
                keccak256(abi.encodePacked((emp)))
            ) {
                w = mailplaceA[i].way;
            }
        }
        ...
        for (i = 0; i <= p_linght; i++) {
            if (
                ...
                packageway[pw_linght] = PackageWay(
                    package[i].tracknumber,
                    package[i].statusways
                );
    ...
    function SendMailB(
        ...
        packagecoin[pc_linght] = PackageCoin(
            tracknumber,
            class,
            DeliveryPeriodReturns(class),
            CostofDeliveryReturns(class),
            weight,
            declaredvalue,
            totalcost
        );
